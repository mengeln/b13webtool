...
if(!file.exists(args[1]))stop("File does not exist")

# file <- "M:/qPCR_data/ent_sketa.csv"
file <- args[1]
eff.max <- 2.1
eff.min <- 1.87
r2.min <- 0.98
...
# sink("/var/scripts/b13micro/files/output.txt", append=FALSE, split=FALSE)

#knit("/var/scripts/b13micro/report.Rtex", paste0("/var/scripts/b13micro/files/", outputName, ".tex"))
knit("/var/scripts/b13micro/report.Rtex", paste0("/var/www/b13micro/files/", outputName, ".tex"))
#system(paste0("xelatex /var/scripts/b13micro/files/, outputName, ".tex -output-directory=/var/scripts/b13micro/files/ -job-name=", outputName))
#unlink(c(paste0("/var/www/b13micro/files/", outputName, c(".aux", ".log")), "/var/www/b13micro/files/report.tex"))
cat("Test Return");
